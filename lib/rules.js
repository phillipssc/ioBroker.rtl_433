                ...
                try {
                    for (let i=0; i<channels.length; i++) {
                        ...
                        if (parts) {
                            ...
                            // Run the filter
                            const ruleObj = await this.routines[ruleType](retObj, args);
                            // Combine the results, if the result removed a key we do too != {...retObj, ...ruleObj} in that sense
                            if (ruleObj !== null) {
                                Object.keys(retObj).forEach(key => {
                                    retObj[key] = ruleObj[key];
                                });
                            } 
                            else {
                                retObj = null;
