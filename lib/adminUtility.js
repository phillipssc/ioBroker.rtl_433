...
class AdminUtility {
      ...
      if (match) {
        ...
        this.adapter.getChannelsOf(devAddr, (err, channels) => {
          ...
          broker.createChannel(dObj, devAddr, typeIteration, (chAddr) => {
            for (let i=0; i<rules[type].inputs.length; i++) {
              const setting = rules[type].inputs[i];
              if (setting.output) {
                ...
                broker.createState(
                  ...
                  (stateAddr) => {
                    ...
                    this.adapter.setState(stateAddr, setting.value, () => {
                      if (i = rules[type].inputs.length - 1) resolve(chAddr);
                    });
                  }
                );
              }
  ...
  removeSettings(args) {
    ...
    if (parts) {
      ...
      const settings = rules[type].inputs;
  ...
  setState(args) {
    return new Promise((resolve, reject) => {
      ...
      const state = parts[1];
      const devAddr = parts[0];
      this.adapter.log.debug(`setState('${devAddr}', ${state})`);
