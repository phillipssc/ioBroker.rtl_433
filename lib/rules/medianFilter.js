function medianFilter(dObj, args) {
    ...
    if (args.field !== '' && dObj[args.field] !== undefined) {
        ...
        args.adapter.log.debug(`medianFilter: starting: ${args.field} data in: ${newDataPoint}`);
        const data = args.filterData[`${dObj.model}-${dObj.id}.${type}.${args.field}`] || [];
        data.push(newDataPoint);
        if (data.length >= parseInt(args.length, 10) + 1) data.shift();
        args.adapter.log.debug(`medianFilter: array: ${JSON.stringify(data)}`);
        args.filterData[`${dObj.model}-${dObj.id}.${type}.${args.field}`] = data;
        retObj[args.field] = median(data);
        args.adapter.log.debug(`medianFilter: completed: ${args.field} data out: ${retObj[args.field]}`);
    }
    return retObj;
}
