...
class BrokerInterface {
  ...
  isExistingDevice(id) {
    ...
    return this.devices.some((device) => {
      const parts = regex.exec(device._id);
      if (parts) {
        ...
        if (newData !== null) {
          Object.keys(dObj).forEach(key => {
            if (dObj[key] !== newData[key]) {
              this.adapter.log.warn(`brokerinterface: rules changed ${dObj.model}-${dObj.id}.${key} from ${dObj[key]} to ${newData[key]}`);
            }
          });
          this.updateDeviceStates(newData);
        }
        else {
          this.adapter.log.warn(`brokerinterface: rules removed ${dObj.model}-${dObj.id} data from datastream`);
        }
      } else {
        this.adapter.log.info('Creating new object');
